...
Public Class DiDi20250526
    ...
    End Structure
    ' Registro 0 (cabecera):
    ' - J(0).lenClau: número total de claves presentes en el índice.
    ' - J(0).reg: puntero al último registro eliminado (baja lógica), enlazado con los anteriores.
    ' Esta información permite reutilizar nodos liberados y mantener un control centralizado.

    ' Ejemplo de uso:
    '   - Al insertar: se incrementa lenClau y se reutiliza un nodo si reg apunta a alguno disponible.
    '   - Al eliminar: se actualiza reg para enlazar el nodo dado de baja a la lista de bajas existentes.
...
retTrue:
    ...
    ' Obtiene un nuevo nodo disponible.
    ' Si hay nodos previamente dados de baja (J(0).reg <> 0), reutiliza el primero de la lista.
    ' Si no hay bajas, incrementa el contador en J(0).Izq y asigna un nuevo número de nodo.
    Private Function GetNode() As Int32
        Dim nnodo As Int32
        If J(0).reg = 0 Then
            ' No hay nodos dados de baja: asigna uno nuevo incrementando el contador en Izq
            J(0).Izq += 1
            nnodo = J(0).Izq
        Else
            ' Reutiliza un nodo dado de baja
            nnodo = J(0).reg
            Dim nd As Nodo = ReadDirectory(J(0).reg)
            J(0).reg = nd.reg ' Actualiza el puntero en el registro 0 al siguiente nodo dado de baja
            nd.reg = 0 ' Limpia el campo reg del nodo reutilizado (opcional pero recomendable)
        End If
        WriteDirectory(J(0), 0) ' Guarda el estado actualizado del registro 0
        Return nnodo
    End Function
...
salida:
    ...
    Private Function remove(ByRef clave As Byte())
        ...
        Me.RemoveNode(Pila(KeyNearest))
        Do While l1 > 0 And nd.sigRegClau > 0
            Me.RemoveNode(nd.sigRegClau)
...
salida:
    ...
    Function ToStringAllKeys() As String
        ...
        Try
            ...
            'NLecturas = 0
            Dim param As New DiDi20250526.SearchParams With {
                    .key = vClave,
                    .register = NAdd,
